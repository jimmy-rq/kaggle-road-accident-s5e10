...
# -------------------------------------------------------------------
# 1. HELPER FUNCTIONS (Copied *directly* from your notebook)
# -------------------------------------------------------------------

# We cache these functions so Streamlit doesn't re-load them on every interaction
@st.cache_data
def load_assets():
    ...
    # Load the original data (needed for the pipeline)
...
def get_unique_values(train_df):
    ...
    for col in categorical_cols:
        try:
            ...
            # Provide some defaults if column is missing, so app doesn't crash
...
# This is your exact function from the notebook
def create_frequency_features(train_df, test_df, cols, num, cat):
    ...
    # Note: In the app, we only care about the processed 'test' frame
    return train, test
...
def add_new_features(train_df, test_df, orig_df=None):
    ...
    # The 'orig_df' part from your notebook wasn't used, so it's omitted here
    return train, test

# -------------------------------------------------------------------
# 2. THE FULL PREDICTION PIPELINE
# -------------------------------------------------------------------
...
def run_prediction_pipeline(input_df, train_df, target):
    ...
    """
    # Make copies to avoid changing the cached data
    train_copy = train_df.copy()
    test_copy = input_df.copy()
    
    # Ensure 'id' column exists for processing, even if it's a placeholder
    if 'id' not in train_copy.columns:
        train_copy['id'] = range(len(train_copy))
    if 'id' not in test_copy.columns:
        test_copy['id'] = [0] # Placeholder ID for the single prediction

    # 1. Identify features (as in your notebook)
    ...
    # 2. Create Frequency/Binning features
    # This function uses train_copy to build stats and applies them to test_copy
    _, test_copy = create_frequency_features(train_copy, test_copy, cols, num, cat)

    # 3. Prepare categorical features
    # Note: The 'train_copy' cat features are already set by the function,
    # but we must set the 'test_copy' features
    for col in cat:
    ...
    # We must ensure these columns exist before trying to drop them
    cols_to_drop = [col for col in remove if col in test_copy.columns]
    test_copy = test_copy.drop(columns=cols_to_drop)

    # 6. Add new features
    # The train_copy is just a placeholder here, we only need the 'test_copy' output
    _, test_copy = add_new_features(train_copy, test_copy)
    
    # 7. Drop ID column before prediction
...
# Get numerical ranges from the training data
# We use these to set logical min/max values for the sliders
...
if st.sidebar.button("Predict Accident Risk"):
    # 1. Collect inputs into a DataFrame (must match original_train.csv)
    input_data = {
        'id': [0], # Placeholder ID
    ...
    # 3. Make prediction
    # We must enable categorical features for the predict step, just like in training
    try:
        ...
        # Add a simple visual
        st.progress(float(prediction))  # Cast to Python float; assumes score is 0-1
        ...
        st.dataframe(processed_input) # Show what was sent to the model for debugging